...
`spatial_model=TRUE` in `fit_BBM()`, as is the default), INLA is required. 
Due to a CRAN policy, INLA cannot be installed automatically. 
...
required for standard BrainMap (with `spatial_model=FALSE` in `fit_BBM()`).
